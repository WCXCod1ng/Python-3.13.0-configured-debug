...
// Do not edit!
        ...
        case _LOAD_FAST: {
            _Py_UopsSymbol *value;
            value = GETLOCAL(oparg); // 根据oparg获取局部变量
            stack_pointer[0] = value;
            stack_pointer += 1; // 压栈
            break;
        }
        ...
        case _LOAD_CONST: {
            _Py_UopsSymbol *value;
            PyObject *val = PyTuple_GET_ITEM(co->co_consts, this_instr->oparg); // 获取常量表中的一个元素
            int opcode = _Py_IsImmortal(val) ? _LOAD_CONST_INLINE_BORROW : _LOAD_CONST_INLINE; // 根据常量是否immortal选择不同的opcode
            REPLACE_OP(this_instr, opcode, 0, (uintptr_t)val); // 替换当前指令的操作码
            OUT_OF_SPACE_IF_NULL(value = sym_new_const(ctx, val)); // 创建一个新的符号对象并存储在value中
            stack_pointer[0] = value;
            stack_pointer += 1; // 压栈
            break;
        }
        ...
        case _BINARY_OP_ADD_INT: {
            ...
            { // 常量传播
                ...
                OUT_OF_SPACE_IF_NULL(res = sym_new_type(ctx, &PyLong_Type)); // 去优化
            }
            stack_pointer[-2] = res;
            stack_pointer += -1;
            break;
        ...
        case _BINARY_SUBSCR: { // 下标访问 obj[index]
            _Py_UopsSymbol *res;
            res = sym_new_not_null(ctx);
            if (res == NULL) goto out_of_space;
            stack_pointer[-2] = res;
        ...
        case _BINARY_SLICE: { // 切片 obj[start:end]，由是静态分析，所以均不涉及实际的动作（取切片、索引等），而是利用一个符号res表示本次操作的结果
            _Py_UopsSymbol *res;
            res = sym_new_not_null(ctx);
            if (res == NULL) goto out_of_space;
            stack_pointer[-3] = res;
