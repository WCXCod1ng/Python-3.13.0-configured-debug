...
)
{
    ...
    int curr_space = 0;
    int max_space = 0; // 最大栈空间
    _PyUOpInstruction *first_valid_check_stack = NULL; // 记录第一个有效的_CHECK_STACK_SPACE指令
    _PyUOpInstruction *corresponding_check_stack = NULL;

    if (_Py_uop_abstractcontext_init(ctx) < 0) { // 初始化ctx，并分配资源，如果失败则跳转到out_of_space标签
        goto out_of_space;
    }
    // 构建抽象解释器栈帧，是为了在不执行代码的情况下，分析代码的行为和特性，从而进行优化，例如：常量折叠、传播，指令合并，移除死代码
    _Py_UOpsAbstractFrame *frame = _Py_uop_frame_new(ctx, co, curr_stacklen, NULL, 0);
    if (frame == NULL) {
        return -1;
    }
    ctx->curr_frame_depth++;
    ctx->frame = frame; // 更新当前帧以及所在的帧深度

    _PyUOpInstruction *this_instr = NULL;
    for (int i = 0; i < trace_len; i++) { // 对buffer中的每个uop，逐一处理
        this_instr = &trace[i];

        int oparg = this_instr->oparg; // 操作参数
        opcode = this_instr->opcode; // 操作码
        _Py_UopsSymbol **stack_pointer = ctx->frame->stack_pointer; // 保存栈指针
...
#include "optimizer_cases.c.h"
        ...
        ctx->frame->stack_pointer = stack_pointer; // 恢复栈指针
        assert(STACK_LEVEL() >= 0);
    }
    Py_UNREACHABLE();

out_of_space: // 栈空间不足，无法优化
    DPRINTF(3, "\n");
...
hit_bottom: // 遇到不可达代码，通常由于栈符号分析中的冲突引发
