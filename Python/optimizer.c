...
// 检查给定的字节码是否有空间插入新的executor
static bool
has_space_for_executor(PyCodeObject *code, _Py_CODEUNIT *instr)
{
    if (instr->op.code == ENTER_EXECUTOR) {// 已经插入过了executor
        return true;
    }
    if (code->co_executors == NULL) { // 之前没有插入executor
        return true;
    }
    return code->co_executors->size < MAX_EXECUTORS_SIZE; // 与预留空间比较
}

// 为新的executor获取一个索引（以便于插入到code->co_executors中）
static int32_t
get_index_for_executor(PyCodeObject *code, _Py_CODEUNIT *instr)
{
    ...
    return size;// size就是要返回的索引值（similar to vector<>.push_back()）
}

// 将executor插入到code的executor列表的指定索引处
static void
insert_executor(PyCodeObject *code, _Py_CODEUNIT *instr, int index, _PyExecutorObject *executor)
{
    ...
    code->co_executors->executors[index] = executor; // insert into position index
    assert(index < MAX_EXECUTORS_SIZE);
    instr->op.code = ENTER_EXECUTOR;
    instr->op.arg = index; // 在arg字段保存executor插入的索引
}
...
_Py_SetOptimizer(PyInterpreterState *interp, _PyOptimizerObject *optimizer)
{
    ...
    _PyOptimizerObject *old = interp->optimizer; // get old one
    if (old == NULL) {
        old = &_PyOptimizer_Default;
    }
    Py_INCREF(optimizer);
    interp->optimizer = optimizer; // set new one
    return old; // return old one
}
    ...
    PyObject **stack_pointer, _PyExecutorObject **executor_ptr)
{
    ...
    PyInterpreterState *interp = _PyInterpreterState_GET();
    if (!has_space_for_executor(code, start)) {// 没有空间插入executor，则不能优化
        return 0;
    }
    // 调用优化器进行优化，得到一个执行器对象（由executor_ptr指向）
    ...
    // 插入executor_ptr指向的执行器到code的executor list中
...
_Py_GetExecutor(PyCodeObject *code, int offset)
{
    ...
    for (int i = 0 ; i < code_len;) {
        if (_PyCode_CODE(code)[i].op.code == ENTER_EXECUTOR && i*2 == offset) {
            ...
            _PyExecutorObject *res = code->co_executors->executors[oparg];// 以oparg为索引
            Py_INCREF(res);
            return res;
        }
...
// 获取_PyExecutorObject中vm_data（_PyVMData）的不同字段
static PyObject *
...
_PyUOpName(int index)
{
    ...
    return _PyOpcode_uop_name[index]; // 定义在pycore_uop_metadata.h的数组，key=opcode，value=uop的名称
}
...
// 获取uop所占的大小（即_PyExecutorObject中code_size）
static Py_ssize_t
...
// 获取一个uop（_PyExecutorObject包含一个_PyUOpInstruction数组，获取指定索引index中的某一项）的详细信息（opcode、oparg、target、operand）
static PyObject *
...
// 获取_PyExecutorObject中的jit_code字段，实际返回PyBytesObject*
static PyObject *
...
// 向uop数组（由_PyUOpInstruction*trace指向）中添加一个uop，并返回更新后的长度
static inline int
add_to_trace(
    _PyUOpInstruction *trace, // 存储uop
    int trace_length,
    uint16_t opcode, // 以下是要添加的uop（_PyUopInstruction）的各个字段
    uint16_t oparg,
    uint64_t operand,
    uint32_t target)
{
    trace[trace_length].opcode = opcode;
    trace[trace_length].format = UOP_FORMAT_TARGET; // 标记该指令的格式
...
top:  // Jump here after _PUSH_FRAME or likely branches
    for (;;) { // 循环解析bytecode
        target = INSTR_IP(instr, code); // target为当前指令相对code的偏移量
        // Need space for _DEOPT
        max_length--;

        uint32_t opcode = instr->op.code;
        uint32_t oparg = instr->op.arg; // 当前指令的opcode和oparg

        DPRINTF(2, "%d: %s(%d)\n", target, _PyOpcode_OpName[opcode], oparg);

        // 处理特殊的一些指令
        if (opcode == ENTER_EXECUTOR) { // 如果当前指令时executor的开始指令，则获取对应的执行器
            assert(oparg < 256);
        ...
        if (opcode == EXTENDED_ARG) { // EXTENDED_ARG是字节码扩展指令，用于处理更大操作数的情况，将后续字节码中的arg合并到当前的oparg中
            instr++;
            opcode = instr->op.code;
            oparg = (oparg << 8) | instr->op.arg;
            if (opcode == EXTENDED_ARG) { // 如果下一个指令还是EXTENDED_ARG
                instr--;
                goto done;
            }
        }
        assert(opcode != ENTER_EXECUTOR && opcode != EXTENDED_ARG);
        RESERVE_RAW(2, "_CHECK_VALIDITY_AND_SET_IP"); // 预留两个空间
        ADD_TO_TRACE(_CHECK_VALIDITY_AND_SET_IP, 0, (uintptr_t)instr, target); // 将_CHECK_VALIDITY_AND_SET_IP插入到uop数组中
        ...
        switch (opcode) {
            ...
            { // 关于分支预测的优化
                ...
                if (jump_likely) { // 调整置信度confidence
            ...
            { // 对于循环的优化
                ...
                if (target == initial_instr) { // 回到了循环起点
            ...
            default:
            { // 对于默认（未实现的opcode）通过宏展开成为uop，并依次处理
...
// 它会在微操作（uops）指令序列中处理和调整隐式的控制流和错误处理，使得它们在后续执行时变得更加明确和易于追踪。这是为了提高代码的执行效率，并确保在执行过程中能够正确地处理各种控制流和异常情况
...
prepare_for_execution(_PyUOpInstruction *buffer, int length)
{
    ...
    int32_t current_exit_op = -1; // 用于跟踪当前的跳转目标、错误处理目标、退出操作符等。它们帮助函数在转换过程中维护隐式控制流的信息。
    /* Leaving in NOPs slows down the interpreter and messes up the stats */
    _PyUOpInstruction *copy_to = &buffer[0];
    for (int i = 0; i < length; i++) {
        _PyUOpInstruction *inst = &buffer[i];
        if (inst->opcode != _NOP) { // 不考虑NOP，其他指令复制到copy_to位置
            if (copy_to != inst) {
                *copy_to = *inst;
            }
            copy_to++;
    ...
    for (int i = 0; i < length; i++) {
        ...
        int32_t target = (int32_t)uop_get_target(inst); // 当前指令的目标地址
        if (_PyUop_Flags[opcode] & (HAS_EXIT_FLAG | HAS_DEOPT_FLAG)) { // 当前指令含有退出标志或去优化标志
            uint16_t exit_op = (_PyUop_Flags[opcode] & HAS_EXIT_FLAG) ?
                _EXIT_TRACE : _DEOPT; // 生成显示的退出指令
            int32_t jump_target = target;
            if (is_for_iter_test[opcode]) { // 涉及到循环的指令
                ...
                jump_target = next_inst + inst->oparg + 1;// 将跳转目标设置为迭代器的处理代码
            }
            if (jump_target != current_jump_target || current_exit_op != exit_op) {// 当前退出指令的目标地址和之前记录的目标不同，会创建一个新的退出指令
        ...
        if (_PyUop_Flags[opcode] & HAS_ERROR_FLAG) { // 错误处理指令：当前指令带有错误处理标志
            int popped = (_PyUop_Flags[opcode] & HAS_ERROR_NO_POP_FLAG) ?
                0 : _PyUop_num_popped(opcode, inst->oparg);
            if (target != current_error_target || popped != current_popped) { // 当前错误指令的目标地址和之前记录的目标不同，会创建一个新的错误指令
...
// 函数 make_executor_from_uops 的作用是将一个包含微操作（uops）的指令缓冲区转换为一个执行器（executor）对象。执行器是用于执行已优化的字节码的结构体。该函数会处理指令中的跳跃、退出、动态退出等，并根据这些指令创建相应的执行器。
...
make_executor_from_uops(_PyUOpInstruction *buffer, int length, const _PyBloomFilter *dependencies)
{
    ...
    for (int i = length-1; i >= 0; i--) {
        ...
        *dest = buffer[i];
        ...
        if (opcode == _EXIT_TRACE) { // 如果是退出追踪指令，将目标设置为当前的退出点，并标记退出指令。
            executor->exits[next_exit].target = buffer[i].target;
            dest->exit_index = next_exit;
            dest->format = UOP_FORMAT_EXIT;
            next_exit--;
        }
        if (opcode == _DYNAMIC_EXIT) { // 处理动态退出指令，确保它们的目标指向正确的位置。
...
// 实际的优化函数，将来通过_PyOptimizer_NewUOpOptimizer()函数设置到_PyOptimizerObject.optimize字段中
static int
uop_optimize(
    _PyOptimizerObject *self, // 优化器对象的引用，表示当前正在优化的优化器实例
    _PyInterpreterFrame *frame,
    _Py_CODEUNIT *instr, // 待优化代码的入口点
    _PyExecutorObject **exec_ptr, // 指向executor的指针
    int curr_stackentries) // 当前Python虚拟机的栈条目数量，用于优化分析时估计栈深度
{
    _PyBloomFilter dependencies;
    _Py_BloomFilter_Init(&dependencies); // 初始化布隆过滤器，用于识别代码依赖关系
    _PyUOpInstruction buffer[UOP_MAX_TRACE_LENGTH]; // 定义一个固定长度的缓冲区（buffer），用于存储翻译之后的微操作
    OPT_STAT_INC(attempts); // 增加优化尝试次数
    // 进行实际的翻译工作，返回length>0表示成功优化的微操作的数量
    ...
    for (int pc = 0; pc < length; pc++) {
        ...
        if (_PyUop_Flags[opcode] & HAS_OPARG_AND_1_FLAG) { // 根据操作数符合的不同条件使用不同策略
...
// 将executor插入到内部维护的executor list的头部
static void
...
// 从内部维护的executor list中移除某个executor
static void
...
executor_clear(_PyExecutorObject *executor)
{
     ...
     */
    Py_INCREF(executor); // 增加执行器的引用计数，防止在后续清理过程中由于引用计数减为零而导致对象被提前释放。这是因为执行器可能与自身形成循环引用
    for (uint32_t i = 0; i < executor->exit_count; i++) { // 遍历所有的退出点
        const _PyExecutorObject *cold = &COLD_EXITS[i]; // 默认的冷退出?
        const _PyExecutorObject *side = executor->exits[i].executor; // 侧退出?
    ...
    _Py_ExecutorDetach(executor); // 移除当前的executor
    Py_DECREF(executor);
    return 0;
}
