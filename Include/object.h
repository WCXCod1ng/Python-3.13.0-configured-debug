...
static inline Py_ALWAYS_INLINE int _Py_IsImmortal(PyObject *op) // 定义对象会被回收
{
#if defined(Py_GIL_DISABLED)
    return (_Py_atomic_load_uint32_relaxed(&op->ob_ref_local) ==
            _Py_IMMORTAL_REFCNT_LOCAL);
#elif SIZEOF_VOID_P > 4
    return (_Py_CAST(PY_INT32_T, op->ob_refcnt) < 0); // 在64位操作系统中，使用负引用计数来标记对象为不死对象（预设的-5~257、list、tuple、dict等对象）
#else
