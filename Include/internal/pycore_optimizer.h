 ...
 */
typedef struct {
    uint16_t opcode:14; // 操作码14位
    uint16_t format:2; // 指令格式类型2位，代表了三种格式
    uint16_t oparg; // 操作数16位
    union {
        uint32_t target;
        struct {
            union {
    ...
    uint64_t operand;  // A cache entry，用于存储额外的信息
} _PyUOpInstruction;
// 根据格式的不同解析出不同的内容
...
typedef struct _exit_data {
    uint32_t target;
    _Py_BackoffCounter temperature; // JIT
...
typedef struct _PyExecutorObject {
    ...
    uint32_t exit_count; // 这个字段记录了某个执行路径中“退出”点的数量，热点代码追踪
    uint32_t code_size; // 原始代码大小
    size_t jit_size; // jit编译后的代码大小
    void *jit_code; // jit编译后的代码
    void *jit_side_entry;
    _PyExitData exits[1];
} _PyExecutorObject;
